...
%   obj：训练后的SCC_AdaBoost对象   
    ...
    for m = 1:num_weak % 迭代开始
        ...
        for k = 1:K
            weak_classifer = SCC_WeakClassifier(@(x)project_function(x,XD(k)),XT(k));
        ...
        if r_max >= 1
            weak_classifer_best = SCC_WeakClassifier(@(x)project_function(x,d_best),t_best);
